...
def place_order(order_type, symbol, volume, price, stop_loss, take_profit, comment):
    ...
    # Tạo yêu cầu
    request = {
        "action": MetaTrader5.TRADE_ACTION_DEAL if order_type in [MetaTrader5.ORDER_TYPE_BUY, MetaTrader5.ORDER_TYPE_SELL] else MetaTrader5.TRADE_ACTION_PENDING,
        "symbol": symbol,
        "volume": volume,
        "type": order_type,
        "price": round(price, 2),  # Làm tròn giá đến 2 chữ số thập phân
        "sl": round(stop_loss, 2),  # Làm tròn SL đến 2 chữ số thập phân
        "tp": round(take_profit, 2),  # Làm tròn TP đến 2 chữ số thập phân
    ...
    # Kiểm tra xem order_result có phải là None không
    if order_result is None:
        print("Error: Order result is None. Check your request parameters and connection.")
        return None

    # Kiểm tra mã trả về
    if order_result.retcode != MetaTrader5.TRADE_RETCODE_DONE:
        print(f"Error placing order. ErrorCode {order_result.retcode}")
        # In chi tiết lỗi
